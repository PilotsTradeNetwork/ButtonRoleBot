...
def _create_button(button_data: RoleButtonData):
    print('_create_button')
...
async def _add_role_button_to_view(interaction: discord.Interaction, button: DynamicButton,
                                   button_data: RoleButtonData, view: discord.ui.View):
    print("Adding dynamic button component")
    print(button)
    view.add_item(button)
    print("Logging to bot-spam")
    embed = discord.Embed(
        description=f"🔘 <@{interaction.user.id}> added a button to {button_data.message.jump_url} to {button_data.button_action} the <@&{button_data.role_id}> role.",
