...
class TestModalView(View):
    def __init__(self, action1, action2):
        self.action1 = action1
        self.action2 = action2
        super().__init__(timeout=None)

    @discord.ui.button(label="One", style=discord.ButtonStyle.secondary, custom_id='button_test_one')
    async def button_one_press(self, interaction: discord.Interaction, button):
        await interaction.response.send_modal(TestModal(self.action1))

    @discord.ui.button(label="Two", style=discord.ButtonStyle.secondary, custom_id='button_test_two')
    async def button_two_press(self, interaction: discord.Interaction, button):
        await interaction.response.send_modal(TestModal(self.action2))
...
class TestModal(Modal):
    def __init__(self, action):
        self.action = action
        self.default = ""
        super().__init__(timeout=None, title=action)
        self.field_1.label=self.action
        self.field_1.default=self.default

    field_1 = discord.ui.TextInput(
        label='Placeholder',
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message(self.action + self.field_1.value, ephemeral=True)
...
class EmbedParamsModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        self.embed_data.embed_thumbnail_url = self.thumbnail.value if self.thumbnail.value else None
        self.embed_data.embed_author_name = self.author_name.value if self.author_name.value else None
        self.embed_data.embed_author_avatar_url = self.author_avatar.value if self.author_avatar.value else None

        # validate URLs
        if not is_valid_extension(self.embed_data.embed_thumbnail_url) and self.embed_data.embed_thumbnail_url is not None:
            error = f"Thumbnail URL not valid: {self.embed_data.embed_thumbnail_url}"
            print(error)
            try:
                raise CustomError(error)
            except Exception as e:
        ...
        if not is_valid_extension(self.embed_data.embed_author_avatar_url) and self.embed_data.embed_author_avatar_url is not None:
            error = f"Author Avatar URL not valid: {self.embed_data.embed_author_avatar_url}"
            print(error)
            try:
                raise CustomError(error)
            except Exception as e:
        ...
        if self.embed_data.embed_thumbnail_url is not None:
        ...
        if self.embed_data.embed_author_name is not None:
            ...
            if self.embed_data.embed_author_avatar_url is not None:
...
class EmbedContentModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        self.embed_data.embed_image_url = self.embed_image_url.value if self.embed_image_url.value else None
        print(self.embed_data)

        # validate URLs
        if self.embed_data.embed_image_url is not None and not validators.url(self.embed_data.embed_image_url):
            error = f"Image URL not valid: {self.embed_data.embed_image_url}"
            print(error)
            try:
                raise CustomError(error)
            except Exception as e:
        ...
        if self.embed_data.embed_image_url is not None:
