...
class DynamicButton(
    ...
    template=r"button:role:(?P<role_id>[0-9]+):message:(?P<message_id>[0-9]+):action:(?P<action>[a-z]+)",
):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(description="⏳ Processing...", color=EMBED_COLOUR_QU)
            ...
            try:
                ...
                if bot_member.top_role <= role or role.managed:
                    ...
                    try:
                        # notify user
                        raise CustomError(
                            f"Sorry, I don't have permission to manage <@&{role.id}>. Please contact a <@&{role_mod()}> or <@&{role_council()}> member."
                        )
                    except Exception as e:
                ...
                embed = discord.Embed(description=f"You {adverb} have the <@&{role.id}> role.", color=EMBED_COLOUR_OK)
